    ...
    ui(new Ui::MainWindow)
{
    ...
    QTimer *changePinState = new QTimer(this);
    changePinState->setObjectName("changePinState");
    connect(changePinState, SIGNAL(timeout()), this, SLOT(onChangePinStateTimer()));
    changePinState->setSingleShot(false);
    changePinState->setInterval(990);
...
void MainWindow::refreshBtn()
{
    if (connectStatus) {
        ...
        ui->pwmBtn->setEnabled(true);
        ui->pwmLineEdit->setEnabled(true);
        ui->pinStateBtn->setEnabled(true);
        ...
        ui->pwmBtn->setEnabled(false);
        ui->pwmLineEdit->setEnabled(false);
        ui->pinStateBtn->setEnabled(false);
    }
}
...
void MainWindow::on_pwmBtn_clicked()
{
    ...
    if (pwm >= 3.f && pwm <= 12.f) {
        ...
        servo.dutyCycle = pwm; // [%]
        qDebug() << "servo: adres =" << servo.address << ", hz ="
                 << servo.frequency << ", pwm =" << servo.dutyCycle;
        ...
        if (setResult == DobotCommunicate_NoError) {
            ...
            if (getResult == DobotCommunicate_NoError) {
                qDebug() << "checkServo: adres =" << checkServo.address << ", hz ="
                         << checkServo.frequency << ", pwm =" << checkServo.dutyCycle;
...
void MainWindow::onChangePinStateTimer()
{
    isGripperOpen = !isGripperOpen;
    float pwm = isGripperOpen ? 5 : 7;
...
void MainWindow::on_pinStateBtn_clicked()
{
    QTimer *changePinState = findChild<QTimer *>("changePinState");

    if (ui->pinStateBtn->text() == "start changing pin state")
    {
        changePinState->start();
        ui->pinStateBtn->setText("stop changing pin state");
    }
    else if (ui->pinStateBtn->text() == "stop changing pin state")
    {
        changePinState->stop();
        ui->pinStateBtn->setText("start changing pin state");
    }
    else ui->pinStateBtn->setText("error");
}
